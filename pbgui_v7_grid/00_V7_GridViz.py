...
st.set_page_config(
    page_title="Crypto Trading Bot Grid Visualizer",  # Title of the app
...
# Grouped Parameter Interface
st.header("📊 Parameter Settings")  # Header for the parameter settings section
st.warning("⚠️ This tool is ignoring trailing entry/closes and ema distances! It's pupose is to visualize the basic grid levels and help you (and me) to understand the PB parameters better.")
st.warning("⚠️ This tool is work in progress! It's not fully functional and does not work as expected. Please use with caution and don't trust the output.")
...
def calculate_entry_grid(start_price, start_balance, wallet_exposure_limit,
    ...
    # st.text(entry_quantities)
    # st.text(entry_costs)
    # st.text(entry_exposures)
...
def calculate_close_grid(start_price, close_grid_min_markup, close_grid_markup_range, close_grid_qty_pct):
    """
    Calculate the close (take-profit) grid levels.

    Parameters:
    - start_price (float): The initial position price.
    - close_grid_min_markup (float): Minimum markup percentage for the close grid.
    - close_grid_markup_range (float): Total markup range percentage for the close grid.
    - close_grid_qty_pct (float): Quantity percentage for each close grid level.

    Returns:
    - Tuple[List[float], List[float]]: Lists of close TP prices and corresponding quantities.
    """
